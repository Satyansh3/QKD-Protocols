...
def alice(host, receiver, alice_basis, alice_bits, alice_encoded, p_eve):
    ...
    global alice_key
    # For Key
    alice_key = alice_key_string(alice_bits, alice_basis, basis_from_bob[0].content, eavesdrop_indices)
    #################################################################################
...
def main():
    ...
    for p_eve in p_eve_values:
        ...
        for _ in range(50):
            ...
            print("This is the final key length", key_length)
            exit()
            secret_key_rate = key_length / KEY_LENGTH
            secret_key_rates.append(secret_key_rate)

            network.stop(True)
        exit()
        avg_secret_key_rate = np.mean(secret_key_rates)
        results.append((p_eve, avg_secret_key_rate))

        print(f"p_eve: {p_eve}, Average Secret Key Rate: {avg_secret_key_rate}")
    # return results

    # Plot the results
    p_eve_values, secret_key_rates = zip(*results)
    
    fig, ax = plt.subplots(1, 2, figsize=(12, 6))

    # Column Chart
    ax[0].bar(p_eve_values, secret_key_rates, color='blue')
    ax[0].set_xlabel('Percentage of Qubits for Eavesdropping Detection')
    ax[0].set_ylabel('Secret Key Rate')
    ax[0].set_title('Secret Key Rate vs Eavesdropping Detection Percentage')
    ax[0].set_xticks(p_eve_values)

    # Line Chart
    ax[1].plot(p_eve_values, secret_key_rates, marker='o', linestyle='-', color='red')
    ax[1].set_xlabel('Percentage of Qubits for Eavesdropping Detection')
    ax[1].set_ylabel('Secret Key Rate')
    ax[1].set_title('Secret Key Rate vs Eavesdropping Detection Percentage')
    ax[1].set_xticks(p_eve_values)
...
if __name__ == '__main__':
    results = main()
    # Results will be a list of tuples (p_eve, secret_key_rate)
